...
do
for repair_num_steps in 20 50 200
do
clarena pipeline=CUL_FULL_EXPR index=amnesiachat_paper/amnesiachat_pmnist_5tasks_hparam cl_paradigm=TIL \
  ...
  output_dir=output_amnesiachat_paper/hyperparameterlatest/pmnist_5tasks_til/amnesiachat/b${buffer_size}_s${repair_num_steps}/seed_${global_seed}/${timestamp} \
  global_seed=${global_seed} +if_run_reforiginal=false
done
done
done